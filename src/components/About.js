...
function About() {
    ...
    const articleStyle = `h-28 w-11/12 md:w-full md:h-48 text-center flex flex-col md:grid gap-0 md:grid-rows-2 md:grid-cols-1 bg-${(theme === "white") ? "sky-900" : "stone-900"} hover:scale-105 duration-200 transition-ease-in-out rounded-b-xl md:rounded-b-3xl overflow-hidden shadow-md shadow-${(theme === "white") ? "black" : "white"}`

    const iconNameBox = `flex  md:flex-col justify-center items-center bg-${(theme === "white") ? "cyan-400" : "stone-200"} h-2/6 md:h-5/6 text-${theme} border-2 md:border-4 border-${(theme === "white") ? "black" : "white"} shadow-2xl rounded-b-xl md:rounded-b-3xl`
    ...
    return (
        <>
            <section id='about' className='container  mx-auto mt-24 lg:mt-32 p-0'>
                <div className={`text-center text-${(theme === "white") ? "blue-700" : "red-600/90"} text-3xl lg:text-6xl`}>About Me</div>
                <div className={`text-${(theme === "white") ? "black" : "white"} grid gap-4 md:gap-0 md:grid-cols-2 md:grid-rows-1 px-8 place-content-center place-items-center`}>
                    ...
                    <div className='text-center grid gap-3 gap-y-6 md:gap-y-4 md:gap-4 md:w-5/6 grid-cols-2 grid-rows-1 place-content-center place-items-center m-0 md:text-xl lg:mt-8 lg:text-2xl' >
